...
void InterTechnoClass::tunein(void)
{
		  int8_t i;
		  
 		 	EIMSK &= ~_BV(CC1100_INT);                 
  		DIGITAL_HIGH( CC1100_CS_DDR, CC1100_CS_PIN ); // CS as output
...
void InterTechnoClass::send_bit(uint8_t bit)
{
  if (bit == 1) {
...
void InterTechnoClass::send_start_V3(void) {
    DIGITAL_HIGH(CC1100_OUT_PORT, CC1100_OUT_PIN);     // High          
	MYDELAY.my_delay_us(it_interval);
 	  DIGITAL_LOW(CC1100_OUT_PORT, CC1100_OUT_PIN);       // Low
...
void InterTechnoClass::send_stop_V3(void) {
    DIGITAL_HIGH(CC1100_OUT_PORT, CC1100_OUT_PIN);     // High          
...
void InterTechnoClass::send_bit_V3(uint8_t bit)
{
	if (bit == 1) {
    DIGITAL_HIGH(CC1100_OUT_PORT, CC1100_OUT_PIN);     // High          
  	MYDELAY.my_delay_us(it_interval_v3);
...
void InterTechnoClass::send (char *in) {	
	int8_t i, j, k;
...
void InterTechnoClass::func(char *in)
{
	if (in[1] == 't') {
