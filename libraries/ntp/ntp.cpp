...
// Network part
void
NtpClass::fill_packet(ntp_packet_t *p)
{
  memset(p, 0, sizeof(*p));
...
NtpClass::init(void)
{
  ...
  gmtoff = erb(EE_IP4_NTPOFFSET);
  erip(ipaddr, EE_IP4_NTPSERVER);
  if(ipaddr[0] == 0 && ipaddr[1] == 0)
    erip(ipaddr, EE_IP4_GATEWAY);
  conn = uip_udp_new(&ipaddr, HTONS(NTP_PORT));
  if(conn == NULL)
    return;
  uip_udp_bind(conn, HTONS(NTP_PORT));
  //ntp_sendpacket();
}
...
ntp_digestpacket()
{
  ...
  f[2] = p->tx_ts.u8[1];
  f[3] = p->tx_ts.u8[0];
  hsec = (uint16_t)(p->tx_ts.u8[4]*125)/256;
  if(eth_debug) {
    DC('n'); DC('r'); func(0);
  }
}
...
uint8_t
NtpClass::dec2bcd(uint8_t in)
{
  return ((in/10)<<4) | (in%10);
}

uint8_t
NtpClass::bcd2dec(uint8_t in)
{
...
void
NtpClass::sec2tm(time_t sec, tm_t *t)
{
  uint8_t m, y;
  sec += (ntp_gmtoff*3600);
...
NtpClass::func(char *in)
{
  ...
  if(t == 1) {
    ...
    if(t&4) {
      DC('.'); display.udec((uint16_t)(hsec*100)/125, 2, '0');
    }
    DNL();
...
void
NtpClass::get(uint8_t data[6])                // for the log function
{
  tm_t t;
  sec2tm(ntp_sec, &t);
