...
uint16 MbusPacketClass::packetSize(uint8 lField)
{
  uint16 nrBytes;
  uint8  nrBlocks;
...
//----------------------------------------------------------------------------------
uint16 MbusPacketClass::byteSize(uint8 Smode, uint8 transmit, uint16 packetSize)
{
  uint16 tmodeVar;
...
//----------------------------------------------------------------------------------
void MbusPacketClass::encodeTXPacket(uint8* pPacket, uint8* pData, uint8 dataSize)
{
  ...
  // e.g. L = dataSize + 10
  *pPacket = dataSize + 10;
  crc = MbusCrc.crcCalc(crc,*pPacket);
  pPacket++;
...
void MbusPacketClass::encodeTXBytesSmode(uint8* pByte, uint8* pPacket, uint16 packetSize)
{
  uint16 bytesEncoded;
  ...
  {
    MbusManch.manchEncode((pPacket + bytesEncoded), (pByte + 2*bytesEncoded));
    bytesEncoded++;
  }
...
//----------------------------------------------------------------------------------
void MbusPacketClass::encodeTXBytesTmode(uint8* pByte, uint8* pPacket, uint16 packetSize)
{
  ...
  while (bytesRemaining)
  {
    ...
    if (bytesRemaining == 1)
    {
      Mbus3outof6.encode3outof6(pPacket, pByte, 1);
      bytesRemaining -= 1;
    }
    ...
    {
      Mbus3outof6.encode3outof6(pPacket, pByte, 0);
...
//    PACKET_CRC_ERROR       2
//----------------------------------------------------------------------------------
uint16 MbusPacketClass::decodeRXBytesSmode(uint8* pByte, uint8* pPacket, uint16 packetSize)
{
  uint16 bytesRemaining;
  uint16 bytesEncoded;
  ...
  while (bytesRemaining)
  {
    decodingStatus = MbusManch.manchDecode(pByte, pPacket);
...
//    PACKET_CRC_ERROR       2
//----------------------------------------------------------------------------------
uint16 MbusPacketClass::decodeRXBytesTmode(uint8* pByte, uint8* pPacket, uint16 packetSize)
{
  ...
  while (bytesRemaining)
  {
    ...
    if (bytesRemaining == 1)
    {
      decodingStatus = Mbus3outof6.decode3outof6(pByte, pPacket, 1);
    ...
    else
    {
      ...
      decodingStatus = Mbus3outof6.decode3outof6(pByte, pPacket, 0);
      ...
      // Perform CRC calculation           
       ...
       {
        crc = MbusCrc.crcCalc(crc, *(pPacket));
        crc = MbusCrc.crcCalc(crc, *(pPacket + 1));
       }
...
uint16 MbusPacketClass::verifyCrcBytesCmodeA(uint8* pByte, uint8* pPacket, uint16 packetSize)
{
  uint16 crc = 0;
  uint16 i = 0;
...
uint16 MbusPacketClass::verifyCrcBytesCmodeB(uint8* pByte, uint8* pPacket, uint16 packetSize)
{
  uint16 crc = 0;
  uint16 i = 0;
  if (packetSize > 128) {
