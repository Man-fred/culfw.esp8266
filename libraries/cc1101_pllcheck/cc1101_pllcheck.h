...
#ifdef HAS_CC1101_PLL_LOCK_CHECK
  uint8_t cc1101_checkPLL(void);				// check CC1101 PLL lock with recalibration if necessary, returns 0 if PLL is in lock
#endif

#ifdef HAS_CC1101_PLL_LOCK_CHECK_RX
  uint8_t cc1101_toRX_PLLcheck(void);			// set CC1101 to RX with recalibration if necessary, returns 0 on success
#endif

#ifdef HAS_CC1101_PLL_LOCK_CHECK_TX
  uint8_t cc1101_toTX_PLLcheck(void);			// set CC1101 to TX with recalibration if necessary, returns 0 on success
#endif

#ifdef HAS_CC1101_RX_PLL_LOCK_CHECK_TASK_WAIT
  void cc1101_RX_check_PLL_wait_task(void);	// check for CC1101 PLL Lock and try recalibration, if not. Wait for RX, if recalibrating.
#endif

#ifdef HAS_CC1101_RX_PLL_LOCK_CHECK_TASK_NOWAIT
  void cc1101_RX_check_PLL_nowait_task(void);	// check for CC1101 PLL Lock and try recalibration, if not. Don't wait for RX, if recalibrating.
#endif

#ifdef HAS_CC1101_TX_PLL_LOCK_CHECK_TASK_WAIT
  void cc1101_TX_check_PLL_wait_task(void);	// check for CC1101 PLL Lock and try recalibration, if not. Wait for TX, if recalibrating.
#endif

#ifdef HAS_CC1101_TX_PLL_LOCK_CHECK_TASK_NOWAIT
  void cc1101_TX_check_PLL_nowait_task(void);	// check for CC1101 PLL Lock and try recalibration, if not. Don't wait for TX, if recalibrating.
#endif

#ifdef HAS_CC1101_WAIT_STATE
  uint8_t cc1101_wait_state(uint8_t state, uint8_t poll_us);	// wait for a CC1101 MARCSTATE to be reached within 256*poll_us, returns 0 if state is reached in time
#endif
