...
uint8_t
PllCheckClass::cc1101_checkPLL(void)  // noansi: returns 0 on success
{
	// check PLL Lock and try to calibrate. Possibly restoring a saved calibration could be faster...
	if (CC1100.cc1100_readReg( CC1100_FSCAL1 ) != 0x3f) return 0;			// PLL in Lock as described in CC1101 doc and errata
...
# ifdef HAS_CC1101_PLL_LOCK_CHECK_MSG_SW
	...
	else CC1100.ccStrobe( CC1100_SNOP );									// wait a moment to give time to switch to IDLE state, this should be enough
# else
	DS_P( PSTR( "PLL0\r\n" ) );										// PLL0 -> PLL Lock problem found. Message gives enough time to switch to IDLE state
# endif
#else
	CC1100.ccStrobe( CC1100_SNOP );										// wait a moment to give time to switch to IDLE state, this should be enough
#endif
	...
	CC1100.ccStrobe( CC1100_SCAL );										// Try Calibration
	...
	if (CC1100.cc1100_readReg( CC1100_FSCAL1 ) != 0x3f) return 0;			// PLL in Lock as described in CC1101 doc and errata, we recovered
...
uint8_t
PllCheckClass::cc1101_wait_state(uint8_t state, uint8_t poll_us)
{
	uint8_t l;
	...
	do
	{
		if (CC1100.cc1100_readReg( CC1100_MARCSTATE ) == state) return 0;	// ok, state reached
		MYDELAY.my_delay_us( poll_us );
	}
	while (l--);
...
uint8_t
PllCheckClass::cc1101_toRX_PLLcheck(void)
{
	uint8_t n;
	...
	do {
		// enable RX
		while ((CC1100.ccStrobe( CC1100_SRX ) & CC1100_STATUS_STATE_BM) != 0x10);	// Set RX until Status Byte indicates RX
...
uint8_t
PllCheckClass::cc1101_toTX_PLLcheck(void)
{
	uint8_t n;
	...
	do {
		// enable TX
		while ((CC1100.ccStrobe( CC1100_STX ) & CC1100_STATUS_STATE_BM) != 0x20);	// Set TX until Status Byte indicates TX
...
void
PllCheckClass::cc1101_RX_check_PLL_wait_task(void)
{
  if (CC1100.cc1100_readReg( CC1100_MARCSTATE ) == MARCSTATE_RX)
  {
	...
	if (CC1100.cc1100_readReg( CC1100_FSCAL1 ) == 0x3f)							// no PLL Lock?  as described in CC1101 errata
	{
		cc1101_checkPLL();													// try calibration to recover, takes about 735us
		while ((CC1100.ccStrobe( CC1100_SRX ) & CC1100_STATUS_STATE_BM) != 0x10);	// Set RX again until Status Byte indicates RX, this will take up to 799us depending on AUTOCAL setting! see cc1101 doc
	}
  }
}
...
void
PllCheckClass::cc1101_RX_check_PLL_nowait_task(void)
{
  if (CC1100.cc1100_readReg( CC1100_MARCSTATE ) == MARCSTATE_RX)
  {
	// try init or recalibration, if stuck in RX State with no PLL Lock as seen in extended read timeout logging
	...
	{
		cc1101_checkPLL();													// try calibration to recover, takes about 735us
		CC1100.ccStrobe( CC1100_SRX );												// Set RX, other tasks will give the wait time to fully switch to RX in background, up to 799us needed, depending on AUTOCAL setting! see doc
	}
  }
}
...
// check if stuck in RX state without PLL Lock and try to recover
void
PllCheckClass::cc1101_TX_check_PLL_wait_task(void)
{
  if (CC1100.cc1100_readReg( CC1100_MARCSTATE ) == MARCSTATE_TX)
  {
	...
	{
		cc1101_checkPLL();													// try calibration to recover, takes about 735us
		while ((CC1100.ccStrobe( CC1100_STX ) & CC1100_STATUS_STATE_BM) != 0x20);	// Set RX again until Status Byte indicates RX, this will take up to 799us depending on AUTOCAL setting! see cc1101 doc
	}
  }
}
...
// check if stuck in RX state without PLL Lock and try to recover, don't wait for RX, just strobe
void
PllCheckClass::cc1101_TX_check_PLL_nowait_task(void)
{
  if (CC1100.cc1100_readReg( CC1100_MARCSTATE ) == MARCSTATE_TX)
  {
	// try init or recalibration, if stuck in RX State with no PLL Lock as seen in extended read timeout logging
	...
	{
		cc1101_checkPLL();													// try calibration to recover, takes about 735us
		CC1100.ccStrobe( CC1100_STX );												// Set RX, other tasks will give the wait time to fully switch to RX in background, up to 799us needed depending on AUTOCAL setting! see doc
	}
  }
}
#endif //HAS_CC1101_TX_PLL_LOCK_CHECK_TASK_NOWAIT

#if !defined(NO_GLOBAL_INSTANCES) && !defined(NO_GLOBAL_CC1101_PLLCHECK)
PllCheckClass PllCheck;
#endif
